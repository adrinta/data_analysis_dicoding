...
st.header("Bike Sharing Dashboard", divider=True)
...
ax.set_title('Data Penyewaan 2011 - 2012', fontsize=30)
...
with col1:
    st.header("Q1: Statistik Penyewa Per Hari", divider=True)
    ...
    sns.barplot(
        filtered_hours_df.groupby('weekday').cnt.mean().reset_index(),
        x='weekday',
        y='cnt'
    )
    ax.set_title('Rata - Rata Penyewa / Hari', fontsize=20)
...
with col2:
    st.header("Q2: Statistik Penyewa Per Jam", divider=True)
    filtered_hours_df = hours_df[(hours_df["dteday"] >= str(start_date)) & (hours_df["dteday"] <= str(end_date))]
    filtered_hours_df['dteday'] =  pd.to_datetime(filtered_hours_df["dteday"])
    ...
    ax.set_title('Rata - Rata Penyewa / Jam', fontsize=20)
